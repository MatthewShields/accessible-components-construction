const Modal = class {
  constructor(s) {
    ...
    // Find the content section for this modal
    this.modal_content_section = this.s.modal.querySelectorAll(this.s.content_section)[0];
  ...
  _init_events() {
    ...
    // If modal has attribute `data-open-page-load="true"` then click its first trigger and set to return back to body
    // if(this.s.modal.getAttribute('data-open-page-load') === true || this.s.modal.getAttribute('data-open-page-load') === 'true') {
    //   this.triggers[0].click();
    //   this.body.tabIndex = 1;
    //   this.clicked_trigger = this.body;
    // }
  }
  ...
  open_modal(e) {
    ...
    // Add active class to modal node
    this.s.modal.classList.add(this.s.modal_active_class);
    ...
    if(this.s.modal.clientHeight < this.s.modal.scrollHeight) {
      this.modal_content_section.scrollTop = 0;
      const topOffset = this.modal_content_section.getBoundingClientRect().top;
      const maxHeight = `calc(100vh - ${(topOffset * 2)+ 'px'})`;
      this.modal_content_section.style.maxHeight = maxHeight;
    }
  ...
  close_modal(e) {
    ...
    if(e === true || e.target === this.backdrop_node || e.target === this.modal_close_button) {
      ...
      // Remove active class from modal node
      this.s.modal.classList.remove(this.s.modal_active_class);
...
for (let i = 0; i < modals.length; i++) {
  new Modal({
    ...
    // The class that shows the modal which should be removed / added
    ...
    // The class / attribute of the modal content section
    content_section: '.modal__content',
